...
<!-- generated-at: 2026-07-23T19:10:08.770Z -->
...
## Implemented Second-Order Refinements — Session 128

- **Project-bound Oracle evidence** — absolute surface binding removed false missing-path verdicts; a live preverify now finds the runtime asset integration in this repo.
- **Pressure peak integrity** — every director sample can raise the observed peak while the bounded receipt still stores only band transitions.
- **Hot-path + clipboard truth** — playtest opt-in is parsed once per mount, and a missing Clipboard API cannot produce a false “copied” state.
- **Progression schema resilience** — owned upgrade tiers clamp to each group’s actual tier count rather than a duplicated magic number.
- **Training as an action** — Replay Training immediately launches a clean guided run instead of silently clearing storage.
- **Player-first front door** — internal art cards left the homepage; play, tools, navigation, install, and save concepts now have distinct, plain-language hierarchy.
- **Runtime character realization** — proprietary optimized operative and Karen art now drives live gameplay with deterministic export, transparent fallback, provenance, and real Chromium captures.

The refreshed ranked list contains only credential-, product-decision-, production-data-, publication-, community-, or physical-device gates. None can be honestly implemented from repo code in this session, so the safe local innovation frontier is exhausted without fabricating evidence.
