...
export default function MobileHUD({
  ...
  return (
    <div style={{ position: "absolute", inset: isMobile ? "0 0 64px" : 0, zIndex: 40, pointerEvents: "none", color: "#FFF", fontFamily: "Inter,system-ui,sans-serif" }}>
      <div style={{ position: "absolute", top: "max(6px, env(safe-area-inset-top))", left: isMobile ? 6 : "50%", right: isMobile ? 6 : "auto", width: isMobile ? "auto" : "min(680px, calc(100vw - 32px))", transform: isMobile ? "none" : "translateX(-50%)", minHeight: 52, display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 6, padding: "5px 6px", border: "1px solid rgba(255,255,255,.16)", borderRadius: 14, background: "rgba(5,8,10,.78)", boxShadow: "0 8px 24px rgba(0,0,0,.28)", backdropFilter: "blur(10px)" }}>
      ...
      {detailsOpen && detailRows.length > 0 && (
        <div style={{ position: "absolute", top: "calc(max(6px, env(safe-area-inset-top)) + 114px)", left: isMobile ? 10 : "50%", right: isMobile ? 10 : "auto", width: isMobile ? "auto" : "min(560px, calc(100vw - 32px))", transform: isMobile ? "none" : "translateX(-50%)", maxHeight: "35dvh", overflowY: "auto", padding: 10, pointerEvents: "all", border: "1px solid rgba(255,255,255,.16)", borderRadius: 14, background: "rgba(4,7,10,.94)", boxShadow: "0 16px 40px rgba(0,0,0,.45)" }}>
      ...
      <div style={{ position: "absolute", bottom: isMobile ? 8 : 18, left: isMobile ? 8 : "50%", right: isMobile ? 8 : "auto", width: isMobile ? "auto" : "min(620px, calc(100vw - 32px))", transform: isMobile ? "none" : "translateX(-50%)", display: "grid", gridTemplateColumns: "minmax(105px, 1fr) auto", alignItems: "end", gap: 8 }}>
        ...
        <div style={{ minWidth: 112, padding: "8px 10px", border: `1px solid ${weapon.color}66`, borderRadius: 11, background: "rgba(4,7,10,.75)", textAlign: "right", backdropFilter: "blur(8px)" }}>
          ...
          {!isMobile && <div style={{ marginTop: 3, color: "#94A0AE", fontSize: 8, fontWeight: 750 }}>R reload · Shift dash · Q grenade</div>}
        </div>
      </div>
