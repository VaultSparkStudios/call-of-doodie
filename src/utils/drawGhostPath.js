// Ghost-path canvas drawing extracted from DeathScreen (S164 diet pass).
// Returns the cleanup function for cancelAnimationFrame.
import { buildGhostKillerMarker } from "./ghostPath.js";
import { ENEMY_TYPES } from "../constants.js";

export function drawGhostPath(canvas, ghostData, replayMode, precisionPeakFrame) {
  const ctx = canvas.getContext("2d");
  const W = canvas.width, H = canvas.height;
  ctx.fillStyle = "#050505"; ctx.fillRect(0, 0, W, H);
  if (ghostData.length < 2) return undefined;
  let rafId = 0;
  ...
  // Draw path in colour-coded segments: green → yellow → red (early → mid → final).
  ...
  if (killerMarker) {
    ...
    ctx.strokeStyle = killerMarker.color + "AA"; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(killerMarker.x, killerMarker.y, 12, 0, Math.PI * 2); ctx.stroke();
    ctx.font = "bold 13px monospace"; ctx.textAlign = "center";
    ctx.shadowColor = "#000"; ctx.shadowBlur = 4; ctx.fillStyle = "#FFF";
    ...
    ctx.fillStyle = killerMarker.color; ctx.font = "bold 8px monospace";
  ...
  [["#00B450","EARLY"],["#FFB000","MID"],["#FF2222","FINAL"]].forEach(([col, label], i) => {
  ...
  if (replayFrames.length >= 2) {
    ...
    const drawReplay = (now) => {
      ...
      for (let i = startIndex + 1; i <= frameIndex; i++) {
        ...
        ctx.lineWidth = 1 + t * 2.5; ctx.stroke();
      }
