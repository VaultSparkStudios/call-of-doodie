...
- Session 169 current truth: src/utils/hazardCaseFiles.js owns stable environmental identity and event-specific counts; the natural-run lazy-analysis browser court enforces seven landmarks; task semantics rejects executable work stranded in the latest historical Session lane; exact production source is ca1324136f8c.
- Session 166 current truth: large-arena spawns use resolved world bounds; SEWER EXTRACTION owns a 1.5× arena and bounded 1.25× pressure scale; `radarModel.js` owns loot/evac projection; secondary death analysis is first-open lazy and must receive its complete model explicitly. Hosted transient-state pixels are the regression authority for that lazy boundary.
