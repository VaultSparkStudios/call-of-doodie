...
## Now
- [ ] [SIL:3] [S58] App.jsx slice 10 — extract bullet-vs-enemy resolver into `src/systems/combatResolution.js` as a pure step fn; preserve deterministic seed behavior (replay validation Phase 2 depends on byte-identical scores from headless resim); migrate lightning chain target selection + pierce/bounce decrement + crit roll; keep React refs and addParticles outside the pure path
- [ ] [SIL:2] [S58] validate-replay Phase 2 — once combat extraction lands, add deterministic headless resim from `seed + input_hash`; quarantine submissions with >2% drift from sim; the heuristic gate stays as a fast-path pre-filter
- [ ] [SIL:2] [S58] Heat Meter visual integration — `drawGame.js` should brighten palette ~5% at heat ≥ 40 and add subtle chromatic-aberration on enemy hits at heat ≥ 70 (music tier swap already wired in App.jsx)
- [ ] [SIL:1] [S58] Per-objective achievement chain — 3 hot zones cleared in a row, 5 bounties killed, escort-cart with 100% HP, lockdown survived under 30s remaining; wires into existing 61-achievement system
