...
- **BOT ROYALE bot count.** `src/modes/botRoyale.js` has spawned `BOT_COUNT = 16` since S165. Four player-facing surfaces continued to claim twelve: the `modeCatalog.js` blurb (in-game mode picker), the `modeCatalog.js` description (`/modes/`), `src/content/fieldManual.js` §5 (`/field-manual/` and the in-app quick reference), and the generated `public/gameplay-contract.json` — this project's published machine-readable claim. `src/config/changelog.js:32` simultaneously advertised "sixteen bots on a scrolling two-screen arena". Both numbers were live at the same time.
- **Why no gate saw it.** `validate-public-contract.mjs` and `check-public-claims.mjs` both passed for the whole period, correctly: the generated artifacts faithfully reproduced their source modules. The source was wrong. A generation check can only prove an artifact matches its input; it cannot prove the input is true. This is the same shape as S174's finding one layer down.
- **Why the "single source of truth" did not help.** `modeCatalog.js` calls itself the single source of truth for game-mode identity, and `modeCatalog.test.js` genuinely pins that — ids, ordering against `REPLAY_MODES`, labels. The numbers embedded in its prose strings were never part of that contract and had no authority behind them at all.
- **Correction to the S167 record.** The S167 TASK_BOARD line "Public roadmap/changelog now describe the sixteen-bot scrolling royale" is true of the roadmap and changelog and false of the mode catalog, field manual, and gameplay contract. Corrected here rather than left standing.
- **Correction to the S174 record.** S174's TRUTH_AUDIT entry states `GENOME_HISTORY.json` "contains duplicate and out-of-order session labels (162 appears twice; a snapshot labelled 166 carries S167 prose)". Measured this session: the duplicates are real (S123 and S162, caused by the control-plane writer's `(date, session)` upsert key). **"Out-of-order" is not supported by the data** — all 50 labels are monotonically non-decreasing; there is no reversed label. The "166 carries S167 prose" case is not mechanically provable either: `overallStatus` legitimately persists unchanged across consecutive sessions, and the same string is shared by 24 of the 50 rows, most certainly correct. Both claims are corrected rather than inherited.
- **Found and recorded, not fixed:** S164, S165, S171 and S172 have no genome snapshot at all — four closeouts that skipped the §3.7 renderer. S174's currency gate reads only the maximum label and structurally cannot see this. The ledger is append-only evidence, so the gap is declared as recorded debt in `ACCEPTED_HISTORICAL_DEFECTS`, not back-filled: fabricating four snapshots would invent closeouts that never ran.
- **Verified NOT a defect:** every other mode number is correct against its runtime constant — BOSS GAUNTLET six bosses, HOLD THE THRONE three thrones at thirty seconds, SEWER EXTRACTION lockdown at 100. `public/field-manual.json` carries no mode prose and had nothing to drift. Recorded so the false positives are not rediscovered.
- **Deliberately excluded from the new gate:** dated changelog entries. The September 3 entry says "twelve bots" because twelve bots is what shipped that day; the September 9 entry records the raise. Rewriting a dated record to satisfy a current constant would falsify history. The exclusion is documented in the gate source.
- Evidence boundary: no participant, physical-device, provider, identity, mail, performance, publication, or lifecycle evidence is claimed this session.

Remedy shipped: `src/config/modeFacts.js` is now the authority for every number quoted in both mode code and mode copy — the runtime imports its constants down from it, prose composes up from it. `check-public-claims.mjs` gained a mode-fact class that reads the **generated** public artifacts, which no test and no prior gate read; it failed on first run against the stale contract, proving the catch rather than asserting it. `scripts/check-genome-ledger.mjs` fails on any new duplicate, gap, or reversed label, and is registered in `schema:lint` and `protocol-drift-check.mjs`.

Overall status: audited the player-facing claims rather than the gates — BOT ROYALE had published both twelve and sixteen bots simultaneously since S165, across four live surfaces, with every gate green. Root-fixed at a single derived authority with executable regression protection over the generated artifacts, S174's genome commitment honoured by measurement (two of its inherited claims corrected as unsupported), fully tested (245 files / 1,490 assertions), 15/15 gates green, and pushed directly to main under standing founder authorization. SPARKED remains NO-GO.
## 2026-09-10 — Session 174 — Measurement-layer truth

Status change: three probes that appeared to cover closeout completeness were found to cover less than they read as covering, and one declared gate had been failing unnoticed. Recorded because this is a source-of-truth status change, not a feature.

- `protocol-drift-check.mjs` reports `- OK scripts/render-state-vector.mjs — renders the canonical closeout state vector`. It asserts the file **exists**. It has never asserted the artifact that renderer produces is current, so a renderer that stopped being invoked passed indefinitely. `context/STATE_VECTOR.json` and `context/GENOME_HISTORY.json` were both three sessions behind (S170 against a ledger at S173) with this line green throughout.
- `check-writeback-currency.mjs` anchors on `context/SELF_IMPROVEMENT_LOOP.md` and reported clean — correctly, for the question it asks ("did a closeout run?"). It cannot answer "did the closeout render everything?", and was never intended to. Not a defect in that probe; a gap between two probes that each looked total.
- `npm run lint:strict` had been exiting 1 since `78efba4` (Dependabot #151, merged during S172). S172 and S173 both recorded "strict lint 0 errors" in the SIL and WORK_LOG. That statement is true of the error count and false of the gate — the gate counts warnings and exits non-zero. S173's WORK_LOG additionally described the warning as "pre-existing unrelated"; it was neither pre-existing (it arrived with the plugin bump one session earlier) nor unrelated (it was the sole cause of the red). Corrected here rather than left in the record.
- `context/PROJECT_STATUS.json` published `silAvg3: 995` while `context/STATE_VECTOR.json` published `silAvg3: 997.7`. Both were hand-entered; neither was compared to the SIL ledger, which makes 995.0 the correct value. Now derived at the single write path, so the two surfaces converge by construction rather than by discipline.
- Verified NOT a status change: `context/STUDIO_MANIFEST.json` and `context/MEMORY_INDEX.md` look stale by git date (S170, S169) and are current. The manifest regenerates byte-identically from PROJECT_STATUS; MEMORY_INDEX has no writer and no session marker — it is a static navigation index. Recorded so the false positive is not rediscovered.
- Known and deliberately unfixed: `context/GENOME_HISTORY.json` contains duplicate and out-of-order session labels (162 appears twice; a snapshot labelled 166 carries S167 prose). The new currency gate takes the maximum snapshot so it cannot be fooled by the disorder, but the ledger itself is not clean and is not represented as clean.

Remedy shipped: `scripts/check-closeout-artifact-currency.mjs` measures each session-keyed closeout artifact against the ledger, names the renderer to re-run, and reports anything it cannot read as `unmeasurable` rather than fresh. Wired into `npm run schema:lint` and registered in `protocol-drift-check.mjs`.

Overall status: audited the measurement layer rather than the backlog — `lint:strict` was red from S172 to S174 while both sessions reported it green, and two SESSION_PROTOCOL §3.7 renderers had not run since S170. All four findings implemented with executable regression protection, fully tested (243 files / 1,466 assertions), 14/14 gates green, and pushed directly to main under standing founder authorization. SPARKED remains NO-GO.
...
- Validation truth — focused implementation 57/57, closeout order/freshness 9/9, lint, schema/architecture, public contract 28/28, deployable build, security/npm audit zero, dependency, assets, media, and cost gates pass. The final root-fixed closeout tree passes 191/191 files and 1,143/1,143 assertions in 126.86 seconds.
...
## 2026-09-10 — Session 170 — Startup, audit-context, process, and staging truth
...
- CI truth — workflow 34435876634 blocked date-bound public route/sitemap drift before deployment. Regeneration plus focused 30/30 contract checks closed it; no failed candidate reached production.
- Production truth — source bb88eb48d57d passed workflow 34436020611 and deployed immutable d9842f42. Immutable and canonical health report the SHA; shell 7/7 both, cutover 5/5, backend 5/5, replay 3/3, leaderboard isolation, and launch surfaces pass.
