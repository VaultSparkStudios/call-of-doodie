...
- **Edge Functions deployed with --no-verify-jwt (Session 38)**: Supabase's infrastructure-level JWT verification was blocking all Edge Function calls with a 401. Root cause: the Supabase project uses the new `sb_publishable_*` key format which is not a JWT, so the default JWT verifier rejects it before function code runs. Both functions (`issue-run-token`, `submit-score`) implement their own auth via `userClient.auth.getUser()` + `clientUid` fallback, making infra-level JWT verification redundant. Fix: added `--no-verify-jwt` to both deploy commands in `deploy-supabase-function.yml`.

- **Cloudflare Worker CSP is path-specific per project (Session 38)**: The `vaultspark-security-headers` Cloudflare Worker applies a base CSP to all `vaultsparkstudios.com/*` routes. Call of Doodie requires additions not needed by other studio projects (`blob:` for GIF, `wss://` for supabase-js, `cloudflareinsights.com` for Cloudflare beacon). Rather than broadening the global policy, the Worker now checks `url.pathname.startsWith('/call-of-doodie/')` and applies a game-specific `COD_CSP` override for that path only. A `normalize()` function strips embedded newlines from header values to make the Worker robust against copy-paste encoding artifacts.
