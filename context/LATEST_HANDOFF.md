...
**Shipped:** replay proof is now player-facing, structured, and trend-aware. `src/utils/replayCommandTrace.js` exports `buildReplayProofReceipt()` plus `buildReplayProofTrend()`; `src/utils/runSubmission.js` attaches `traceReceipt` to valid trace-backed submissions; `src/systems/runSession.js` stores compact proof receipts in the last-10 local run history; `src/components/DeathScreen.jsx` renders the REPLAY PROOF card and stamps share-card images with proof trend context; `src/utils/replayResim.js` exports `buildReplayPressureProfile()` and `runResim()` includes the profile receipt; `src/utils/replayTraceFixtures.js` provides rich/basic/weak/malformed fixtures plus a fixture table; `scripts/validate-replay-trace-fixtures.mjs` validates fixture evidence and pressure-profile parity.

**Validation:** focused `npx vitest run src/utils/replayCommandTrace.test.js src/utils/replayResim.test.js src/utils/runSubmission.test.js src/systems/runSession.test.js` passed 28/28; `node scripts/validate-replay-trace-fixtures.mjs` passed 4 fixtures after sandbox retry outside the Windows decrypt failure; full `npm test` passed 450/450; `npm run lint` reported 0 errors / 8 existing warnings; `npm run build` passed.

**Next session:** True physics-parity replay resim remains the larger trust milestone. The next best in-repo slices are wiring the fixture validator into edge-function parity checks, continuing App.jsx extraction around replay/death submission, or resolving the existing lint-warning debt if a clean warning baseline becomes launch-critical.
