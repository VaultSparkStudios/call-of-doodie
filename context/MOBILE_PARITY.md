...
Last reviewed: 2026-09-08 (Session 166)

Call of Doodie's public browser surfaces meet the current CANON-041 engineering-release parity bar:

- The isolated staging matrix at `https://session-166-staging.call-of-doodie.pages.dev/` passed 969/969 checks across the public route set, both `sewer-night` and `porcelain-day` themes, and 390px, 768px, and 1440px widths.
- The touched extraction-loot, open-evacuation, and first-open secondary-analysis states passed 40/40 checks at 390px and 1440px in both themes. Direct rendered-pixel review found no clipped action, horizontal overflow, illegible objective marker, contrast failure, or contained lazy-panel error.
...
The Studio's generic responsive helper skipped because it imports the standalone `playwright` package while this project intentionally pins `@playwright/test`. The skip is not represented as a pass; the project-owned pinned-Chromium matrix and 18-file hash-bound `docs/visual-qa/LATEST.json` receipt are the authoritative automated and subjective evidence.
